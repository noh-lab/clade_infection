...
mypalette <- colorRampPalette(c("#ff0000","#FFa200","#00CC00","#009999")) # http://paletton.com
...
pca.strain$burk <- factor(pca.strain$burk, levels = c("none", "b11", "b859", "b70", "b159"))

ggplot(pca.strain, aes(x=PC1, y=PC2, fill=burk, shape=host)) + geom_point(size=3) + scale_shape_manual(values=c(21,22,24)) + scale_fill_manual(values=c("white", mypalette(4))) + manuscript_theme

ggplot(pca.strain, aes(x=PC1, y=PC2, fill=burk, shape=host)) + geom_point(size=3, position = "jitter") + scale_shape_manual(values=c(21,22,24)) + scale_fill_manual(values=c("white", mypalette(4))) + manuscript_theme + xlab(paste0("PC1: ",percentVar[1],"% variance")) + ylab(paste0("PC2: ",percentVar[2],"% variance")) + stat_ellipse(data=pca.strain, aes(group=pca.strain$host)) 
...
de.strain$res.d159 <- res.strain159$log2FoldChange
de.strain$res.d70 <- res.strain70$log2FoldChange
de.strain$res.d859 <- res.strain859$log2FoldChange
de.strain$res.d11 <- res.strain11$log2FoldChange
temp <- de.strain[,2:9]
de.strain <- temp
limma::vennDiagram(de.strain[,1:4]) # 80 DE genes shared by all
...
de.strain$lfc.1 <- 1/2*(de.strain$res.d159 + de.strain$res.d70) # additive fold change in nonreduced
de.strain$lfc.2 <- 1/2*(de.strain$res.d859 + de.strain$res.d11) # additive fold change in reduced
...
c1 <- ggplot(subset(de.strain, comb=="neither"), aes(x=lfc.1, y=lfc.2)) + geom_point() + geom_abline(intercept = 0, slope = 1, lty="dashed") + ggtitle("(c) not DE") + xlim(-8,12) + ylim(-10,8) + xlab("response to nonreduced\ngenome symbiont") + ylab("response to reduced\ngenome symbiont") + manuscript_theme

c2 <- ggplot(subset(de.strain, comb=="reduced only"), aes(x=lfc.1, y=lfc.2)) + geom_point() + geom_abline(intercept = 0, slope = 1, lty="dashed") + geom_abline(intercept = 0, slope = coef(reg.on2)[1], color=mypalette(4)[1]) + ggtitle("(c) reduced only DE") + xlim(-8,12) + ylim(-10,8) + xlab("response to nonreduced\ngenome symbiont") + ylab("response to reduced\ngenome symbiont") + manuscript_theme

c3 <- ggplot(subset(de.strain, comb=="nonreduced only"), aes(x=lfc.1, y=lfc.2)) + geom_point() + geom_abline(intercept = 0, slope = 1, lty="dashed") + geom_abline(intercept = 0, slope = coef(reg.on1)[1], color=mypalette(4)[4]) + ggtitle("(b) nonreduced only DE") + xlim(-8,12) + ylim(-10,8) + xlab("response to nonreduced\ngenome symbiont") + ylab("response to reduced\ngenome symbiont") + manuscript_theme

c4 <- ggplot(subset(de.strain, comb=="both"), aes(x=lfc.1, y=lfc.2)) + geom_point() + geom_abline(intercept = 0, slope = 1, lty="dashed") + geom_abline(intercept = 0, slope = coef(reg.bo1)[1], color=mypalette(4)[4]) + ggtitle("(b) both DE (lenient)") + xlim(-8,12) + ylim(-10,8) + xlab("response to nonreduced\ngenome symbiont") + ylab("response to reduced\ngenome symbiont") + manuscript_theme
...
c5 <- ggplot(subset(de.strain, comb2=="both"), aes(x=lfc.1, y=lfc.2)) + geom_point() + geom_abline(intercept = 0, slope = 1, lty="dashed") + geom_abline(intercept = 0, slope = coef(reg.bo2)[1], color=mypalette(4)[4]) + ggtitle("(a) both DE (strict)") + xlim(-8,12) + ylim(-10,8) + xlab("response to nonreduced\ngenome symbiont") + ylab("response to reduced\ngenome symbiont") + manuscript_theme


grid.arrange(c5, c4, c1, ncol=2)


setEPS(width=6, height=5) # default is 7x7
postscript(file=paste("dicty.PCA",format(Sys.time(),"%Y%m%d"),"eps",sep="."))
ggplot(pca.strain, aes(x=PC1, y=PC2, fill=burk, shape=host)) + geom_point(size=3, position = "jitter") + scale_shape_manual(values=c(21,22,24)) + scale_fill_manual(values=c("white", mypalette(4))) + manuscript_theme + xlab(paste0("PC1: ",percentVar[1],"% variance")) + ylab(paste0("PC2: ",percentVar[2],"% variance")) + stat_ellipse(data=pca.strain, aes(group=pca.strain$host)) 
dev.off()


setEPS(width=8, height=7) # default is 7x7
postscript(file=paste("dicty.clade.expression",format(Sys.time(),"%Y%m%d"),"eps",sep="."))
grid.arrange(c5, c4, c1, ncol=2)
dev.off()


# no test for "neither" genes, they are not DE
# test for lenient genes, is the magnitude of foldchange larger in response to nonreduced vs. reduced genome symbiont
both.wide <- data.frame(id = c(paste0(rownames(subset(de.strain, comb=="both")),".nr"),paste0(rownames(subset(de.strain, comb=="both")),".re")),
                        lfc.1 = c(subset(de.strain, comb=="both")[["res.d159"]], subset(de.strain, comb=="both")[["res.d70"]]),
                        lfc.2 = c(subset(de.strain, comb=="both")[["res.d859"]], subset(de.strain, comb=="both")[["res.d11"]])
)

# could test if variance is different
both.long <- both.wide %>%
  gather(key = "genome", value = "raw_lfc", lfc.1, lfc.2)
head(both.long, 3)

car::leveneTest(raw_lfc ~ as.factor(genome), data = both.long) # it is

# alternatively test if absolute foldchange is different
both.long$abs_lfc <- abs(both.long$raw_lfc) 

both.wilcox <- both.long  %>%
  wilcox_test(abs_lfc ~ genome) %>%
  add_significance()
both.wilcox # these are different

both.long  %>%
  wilcox_effsize(abs_lfc ~ genome) # small effect size

# test for strict genes, is the magnitude of foldchange larger in response to nonreduced vs. reduced genome symbiont
strict.wide <- data.frame(id = c(paste0(rownames(subset(de.strain, comb2=="both")),".nr"),paste0(rownames(subset(de.strain, comb2=="both")),".re")),
                          lfc.1 = c(subset(de.strain, comb2=="both")[["res.d159"]], subset(de.strain, comb2=="both")[["res.d70"]]),
                          lfc.2 = c(subset(de.strain, comb2=="both")[["res.d859"]], subset(de.strain, comb2=="both")[["res.d11"]])
)

# could test if variance is different
strict.long <- strict.wide %>%
  gather(key = "genome", value = "raw_lfc", lfc.1, lfc.2)
head(strict.long, 3)

car::leveneTest(raw_lfc ~ as.factor(genome), data = strict.long) # it is

# alternatively test if absolute foldchange is different
strict.long$abs_lfc <- abs(strict.long$raw_lfc) 

strict.wilcox <- strict.long  %>%
  wilcox_test(abs_lfc ~ genome) %>%
  add_significance()
strict.wilcox # these are different
