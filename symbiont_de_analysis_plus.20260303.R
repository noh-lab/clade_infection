...
library(BiocManager)
libraries <- c("DESeq2", "ggplot2", "RColorBrewer", "GenomicFeatures", "topGO", "GSEABase", "gridExtra", "Rgraphviz", "rstatix")
...
# GFF files examined to determine which gene starts the second replicon
#PAGRI_04307_gene -> 4033
grep("PAGRI_04307", rownames(res.b159))
#PA070_04249_gene -> 3775
grep("PA070_04249", rownames(res.b70))
#PBONN_02845_gene -> 2683
grep("PBONN_02845", rownames(res.b859))
#PHAYL_02985_gene -> 2880
grep("PHAYL_02985", rownames(res.b11))

# create data tables
chr.b159 <- data.frame(matrix(NA, ncol=1, nrow=nrow(res.b159)))
chr.b159[,1] <- rownames(res.b159)
colnames(chr.b159)[1] <- "gene"
chr.b159$replicon <- factor(c(rep("chromosome",4032), rep("chromid",nrow(res.b159)-4032)), levels=c("chromosome","chromid"))
chr.b159$log2fc <- res.b159$log2FoldChange

chr.b70 <- data.frame(matrix(NA, ncol=1, nrow=nrow(res.b70)))
chr.b70[,1] <- rownames(res.b70)
colnames(chr.b70)[1] <- "gene"
chr.b70$replicon <- factor(c(rep("chromosome",3774), rep("chromid",nrow(res.b70)-3774)), levels=c("chromosome","chromid"))
chr.b70$log2fc <- res.b70$log2FoldChange

chr.b859 <- data.frame(matrix(NA, ncol=1, nrow=nrow(res.b859)))
chr.b859[,1] <- rownames(res.b859)
colnames(chr.b859)[1] <- "gene"
chr.b859$replicon <- factor(c(rep("chromosome",2682), rep("chromid",nrow(res.b859)-2682)), levels=c("chromosome","chromid"))
chr.b859$log2fc <- res.b859$log2FoldChange

chr.b11 <- data.frame(matrix(NA, ncol=1, nrow=nrow(res.b11)))
chr.b11[,1] <- rownames(res.b11)
colnames(chr.b11)[1] <- "gene"
chr.b11$replicon <- factor(c(rep("chromosome",2879), rep("chromid",nrow(res.b11)-2879)), levels=c("chromosome","chromid"))
chr.b11$log2fc <- res.b11$log2FoldChange


# chromids would have increased transcription during symbiotic association 
# log2FC > 0 for chromid genes, no pattern for chromosome genes
# test if foldchange is different by chromosome

b159.wilcox <- chr.b159  %>%
  wilcox_test(log2fc ~ replicon) %>%
  add_significance()
b159.wilcox # these are different 3.28e-31

chr.b159  %>%
  wilcox_effsize(log2fc ~ replicon) # small effect size 0.136


b70.wilcox <- chr.b70  %>%
  wilcox_test(log2fc ~ replicon) %>%
  add_significance()
b70.wilcox # these are different 4.77e-50

chr.b70  %>%
  wilcox_effsize(log2fc ~ replicon) # small effect size 0.178


b859.wilcox <- chr.b859  %>%
  wilcox_test(log2fc ~ replicon) %>%
  add_significance()
b859.wilcox # these are not different 0.758 


b11.wilcox <- chr.b11  %>%
  wilcox_test(log2fc ~ replicon) %>%
  add_significance()
b11.wilcox # these are not different 0.589

# visualize foldchange by replicon
chr.plot1 <- ggplot(chr.b159, aes(x=replicon, y=log2fc)) + geom_violin(aes(fill=factor(replicon))) + stat_summary(fun.y=median, geom="point", size=2) + ylim(-7.5,13.5) + scale_fill_manual(values=c("#009999","#007676"), guide=F)  + manuscript_theme + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) +  ggtitle("(a) P. agricolaris ba159")

chr.plot2 <- ggplot(chr.b70, aes(x=replicon, y=log2fc)) + geom_violin(aes(fill=factor(replicon))) + stat_summary(fun.y=median, geom="point", size=2) + ylim(-7.5,13.5) + scale_fill_manual(values=c("#00CC00","#009e00"), guide=F) + manuscript_theme + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) +  ggtitle("(b) P. agricolaris ba70")

chr.plot3 <- ggplot(chr.b859, aes(x=replicon, y=log2fc)) + geom_violin(aes(fill=factor(replicon))) + stat_summary(fun.y=median, geom="point", size=2) + ylim(-7.5,13.5) + scale_fill_manual(values=c("#FFa200","#c57d00"), guide=F) + manuscript_theme + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) +  ggtitle("(c) P. bonniea bb859")

chr.plot4 <- ggplot(chr.b11, aes(x=replicon, y=log2fc)) + geom_violin(aes(fill=factor(replicon))) + stat_summary(fun.y=median, geom="point", size=2) + ylim(-7.5,13.5) + scale_fill_manual(values=c("#FF0000","#c50000"), guide=F) + manuscript_theme + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) + theme(axis.title.x = element_blank(), axis.title.y = element_blank()) +  ggtitle("(d) P. hayleyella bh11")

grid.arrange(chr.plot1, chr.plot2, chr.plot3, chr.plot4, ncol=2)

setEPS(width=8, height=7) # default is 7x7
postscript(file=paste("burk.replicon.expression",format(Sys.time(),"%Y%m%d"),"eps",sep="."))
grid.arrange(chr.plot1, chr.plot2, chr.plot3, chr.plot4, ncol=2)
dev.off()


# genes necessary for association would be enriched on the chromid
# testing both all DE genes, as well as only upregulated genes
# set up hyperG test order: (gene list in target list (chromid), universe genes in target list (chromid), universe genes not in target list (chromosome), gene list size (all DE genes))
hg.test <- function(a,b,c,d) { min(1-cumsum(dhyper(0:(a-1),b,c,d))) }

chr.b159$de <- c(res.b159$padj <=0.05)
mylist.1 <- subset(chr.b159, de=="TRUE" & replicon=="chromid")[,1]
mylist.2 <- subset(chr.b159, replicon=="chromid")[,1]
mylist.3 <- subset(chr.b159, replicon!="chromid")[,1]
mylist.4 <- subset(chr.b159, de=="TRUE")[,1]

chr.b159$up <- c(res.b159$padj <=0.05 & res.b159$log2FoldChange>0)
mylist.5 <- subset(chr.b159, up=="TRUE" & replicon=="chromid")[,1]
mylist.6 <- subset(chr.b159, up=="TRUE")[,1]

b159.chr.table <- data.frame(Term=c("Association genes on chromid (all)","Association genes on chromid (up)"),
                             ExpCount=c(length(mylist.4)*length(mylist.2)/length(c(mylist.2,mylist.3)), length(mylist.6)*length(mylist.2)/length(c(mylist.2,mylist.3))),
                             ObsCount=c(length(mylist.1), length(mylist.5)))

b159.chr.table$raw.p <- c(hg.test(length(mylist.1), length(mylist.2), length(mylist.3), length(mylist.4)),
                          hg.test(length(mylist.5), length(mylist.2), length(mylist.3), length(mylist.6)))
b159.chr.table$adj.p <- p.adjust(b159.chr.table$raw.p, method="BH", n=length(b159.chr.table$raw.p))

# print final table to screen
b159.chr.table


chr.b70$de <- c(res.b70$padj <=0.05)
mylist.1 <- subset(chr.b70, de=="TRUE" & replicon=="chromid")[,1]
mylist.2 <- subset(chr.b70, replicon=="chromid")[,1]
mylist.3 <- subset(chr.b70, replicon!="chromid")[,1]
mylist.4 <- subset(chr.b70, de=="TRUE")[,1]

chr.b70$up <- c(res.b70$padj <=0.05 & res.b70$log2FoldChange>0)
mylist.5 <- subset(chr.b70, up=="TRUE" & replicon=="chromid")[,1]
mylist.6 <- subset(chr.b70, up=="TRUE")[,1]

b70.chr.table <- data.frame(Term=c("Association genes on chromid (all)","Association genes on chromid (up)"),
                            ExpCount=c(length(mylist.4)*length(mylist.2)/length(c(mylist.2,mylist.3)), length(mylist.6)*length(mylist.2)/length(c(mylist.2,mylist.3))),
                            ObsCount=c(length(mylist.1), length(mylist.5)))

b70.chr.table$raw.p <- c(hg.test(length(mylist.1), length(mylist.2), length(mylist.3), length(mylist.4)),
                         hg.test(length(mylist.5), length(mylist.2), length(mylist.3), length(mylist.6)))
b70.chr.table$adj.p <- p.adjust(b70.chr.table$raw.p, method="BH", n=length(b70.chr.table$raw.p))

# print final table to screen
b70.chr.table


chr.b859$de <- c(res.b859$padj <=0.05)
mylist.1 <- subset(chr.b859, de=="TRUE" & replicon=="chromid")[,1]
mylist.2 <- subset(chr.b859, replicon=="chromid")[,1]
mylist.3 <- subset(chr.b859, replicon!="chromid")[,1]
mylist.4 <- subset(chr.b859, de=="TRUE")[,1]

chr.b859$up <- c(res.b859$padj <=0.05 & res.b859$log2FoldChange>0)
mylist.5 <- subset(chr.b859, up=="TRUE" & replicon=="chromid")[,1]
ylist.6 <- subset(chr.b859, up=="TRUE")[,1]

b859.chr.table <- data.frame(Term=c("Association genes on chromid (all)","Association genes on chromid (up)"),
                             ExpCount=c(length(mylist.4)*length(mylist.2)/length(c(mylist.2,mylist.3)), length(mylist.6)*length(mylist.2)/length(c(mylist.2,mylist.3))),
                             ObsCount=c(length(mylist.1), length(mylist.5)))

b859.chr.table$raw.p <- c(hg.test(length(mylist.1), length(mylist.2), length(mylist.3), length(mylist.4)),
                          hg.test(length(mylist.5), length(mylist.2), length(mylist.3), length(mylist.6)))
b859.chr.table$adj.p <- p.adjust(b859.chr.table$raw.p, method="BH", n=length(b859.chr.table$raw.p))

# print final table to screen
b859.chr.table


chr.b11$de <- c(res.b11$padj <=0.05)
mylist.1 <- subset(chr.b11, de=="TRUE" & replicon=="chromid")[,1]
mylist.2 <- subset(chr.b11, replicon=="chromid")[,1]
mylist.3 <- subset(chr.b11, replicon!="chromid")[,1]
mylist.4 <- subset(chr.b11, de=="TRUE")[,1]

chr.b11$up <- c(res.b11$padj <=0.05 & res.b11$log2FoldChange>0)
mylist.5 <- subset(chr.b11, up=="TRUE" & replicon=="chromid")[,1]
mylist.6 <- subset(chr.b11, up=="TRUE")[,1]

b11.chr.table <- data.frame(Term=c("Association genes on chromid (all)","Association genes on chromid (up)"),
                            ExpCount=c(length(mylist.4)*length(mylist.2)/length(c(mylist.2,mylist.3)), length(mylist.6)*length(mylist.2)/length(c(mylist.2,mylist.3))),
                            ObsCount=c(length(mylist.1), length(mylist.5)))

b11.chr.table$raw.p <- c(hg.test(length(mylist.1), length(mylist.2), length(mylist.3), length(mylist.4)),
                         hg.test(length(mylist.5), length(mylist.2), length(mylist.3), length(mylist.6)))
b11.chr.table$adj.p <- p.adjust(b11.chr.table$raw.p, method="BH", n=length(b11.chr.table$raw.p))
...
b159.sec <- read.table("ba159.secretion.system.plus.txt", h=F, sep="\t")
names(b159.sec) <- c("id","product")

b70.sec <- read.table("ba70.secretion.system.plus.txt", h=F, sep="\t")
names(b70.sec) <- c("id","product")

b859.sec <- read.table("bb859.secretion.system.plus.txt", h=F, sep="\t")
names(b859.sec) <- c("id","product")

b11.sec <- read.table("bh11.secretion.system.plus.txt", h=F, sep="\t")
names(b11.sec) <- c("id","product")

# are secretion system genes found in DE genes? most upregulated, few downregulated
b159.sec[b159.sec$id %in% genes.b159down,]
b70.sec[b70.sec$id %in% genes.b70down,]
b859.sec[b859.sec$id %in% genes.b859down,]
b11.sec[b11.sec$id %in% genes.b11down,] # second t3ss is down-regulated
...
b159.euk[b159.euk$id %in% genes.b159down,]
b70.euk[b70.euk$id %in% genes.b70down,]
b859.euk[b859.euk$id %in% genes.b859down,]
b11.euk[b11.euk$id %in% genes.b11down,]
